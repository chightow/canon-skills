...
A ticket is a folder in your repo (`.tickets/<id>/`), not a card in someone's cloud. Each sprint has three docs: `acceptance.md` for done criteria and tests, `plan.md` for approach and decisions, and `summary.md` (written at close) for the plan-vs-actual record. Durable choices can land in `DECISIONS.md`; cross-session context lives in `HANDOFF.md`. All of it is markdown on disk, with no account, no remote, and no SaaS.
